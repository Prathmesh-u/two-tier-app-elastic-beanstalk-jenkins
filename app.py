...
# Read secret key from environment (fallback only for local dev)
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'dev-secret-change-me')

# 1) Try explicit SQLALCHEMY_DATABASE_URI (you can set this manually if you want)
db_uri = os.getenv('SQLALCHEMY_DATABASE_URI')

# 2) If not set, but RDS_* vars exist (Beanstalk with attached RDS), build URI from them
if not db_uri and os.getenv('RDS_HOSTNAME'):
    db_uri = (
        f"mysql+pymysql://{os.getenv('RDS_USERNAME')}:{os.getenv('RDS_PASSWORD')}"
